...
static TRef snap_pref(jit_State *J, GCtrace *T, SnapEntry *map, MSize nmax,
		      BloomFilter seen, IRRef ref, TRef *heavy_restores,
		      int restore)
{
  IRIns *ir = &T->ir[ref];
  TRef tr;
  if (irref_isk(ref))
    ...
    if (tr == 0) {
      op1 = ir->op1;
      if (op1 >= T->nk)
	op1 = snap_pref(J, T, map, nmax, seen, op1, heavy_restores, restore);
      op2 = ir->op2;
      if (op2 >= T->nk)
	op2 = snap_pref(J, T, map, nmax, seen, op2, heavy_restores, restore);
      if (LJ_HASFFI && ir->o == IR_CNEWI) {
	if (LJ_32 && ref+1 < T->nins && (ir+1)->o == IR_HIOP) {
	  TRef pref = snap_pref(J, T, map, nmax, seen, (ir+1)->op2,
				  heavy_restores, restore);
	  if (restore) {
	...
	for (irs = ir+1; irs < irlast; irs++)
	  ...
	  skip_newref:
	    val = snap_pref(J, T, map, nmax, seen, irs->op2,
			    heavy_restores, restore);
	    if (val == 0) {
	      IRIns *irc = &T->ir[irs->op2];
	      lj_assertJ((irc->o == IR_CONV && irc->op2 == IRCONV_NUM_INT) ||
			 irc->o == IR_TNEW || irc->o == IR_TDUP,
			 "sunk store for parent IR %04d with bad op %d",
			 ref - REF_BIAS, irc->o);
	      val = snap_pref(J, T, map, nmax, seen, irc->op1,
			      heavy_restores, restore);
		...
		TRef pref = snap_pref(J, T, map, nmax, seen, (irs+1)->op2,
					 heavy_restores, restore);
		if (restore)
...
void lj_snap_replay(jit_State *J, GCtrace *T)
{
  ...
  if (pass23) {
	...
	J->slot[snap_slot(sn)] = snap_pref(J, T, map, nent, seen, refp,
					     heavy_restores, 1);
      }
    }
  }
			    ...
			    IRRef ref, TValue *o, TValue *heavy_restores)
{
  ...
  if (ir->r == RID_SUNK) {
    lj_assertJ(ir->s > 0, "heavy sunk allocation has no identity index");
			...
			IRIns *ir, TValue *o, TValue *heavy_restores)
{
  ...
  if (ir->s > 0 && !tvisnil(&heavy_restores[ir->s - 1])) {
