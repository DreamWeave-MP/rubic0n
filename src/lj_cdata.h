...
static LJ_AINLINE GCcdata *lj_cdata_new(CTState *cts, CTypeID id, CTSize sz)
{
  ...
  lj_gc_stats_inc(G(cts->L), new_cdata_calls);
  lj_gc_stats_add(G(cts->L), new_cdata_bytes, sizeof(GCcdata) + sz);
  lj_gc_stats_add(G(cts->L), new_cdata_payload_bytes, sz);
  cd->gct = ~LJ_TCDATA;
...
static LJ_AINLINE GCcdata *lj_cdata_new_(lua_State *L, CTypeID id, CTSize sz)
{
  ...
  lj_gc_stats_inc(G(L), new_cdata_calls);
  lj_gc_stats_add(G(L), new_cdata_bytes, sizeof(GCcdata) + sz);
  lj_gc_stats_add(G(L), new_cdata_payload_bytes, sz);
  cd->gct = ~LJ_TCDATA;
  cd->ctypeid = id;
  return cd;
}
